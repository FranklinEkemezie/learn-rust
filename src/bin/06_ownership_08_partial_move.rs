...
be moved while other parts stay. In such a case, the parent variable cannot be
used afterward as a whole, however the parts that are only reference (and not
...
fn main() {
    ...
    struct Person {
        name: String,
        age: Box<u8>, // stores the `u8` in the heap
    }
